...
const DesignConfigurator = ({
  ...
  async function saveConfiguration() {
    try {
      ...
      const file = new File([blob], "yourNewPhoneCase.png", {type: 'image/png'})

      await startUpload([file], {configId})
    } catch (error) {
      toast({
        title: "Something went wrong",
        description: "There was problem saving your config, Please try again later",
        variant: 'destructive'
      })
    }
  }
                    ...
                    <RadioGroup key={name} value={options[name]} onChange={(val) => {
                      ...
                      <div className="mt-3 space-y-4">
                        {selectableOptions.map((option) => (
                          <Radio key={option.value} value={option} className={({ checked, disabled }) => cn("relative block cursor-pointer rounded-lg bg-white px-6 py-4 shadow-sm border-2 border-zinc-200 focus:outline-none ring-0 focus:ring-0 outline-none sm:flex sm:justify-between", {
